...
for AUDIOFILE in $INPUTFOLDER/$FILESEARCHSTRING; do
	...
	if $DOAUDIONORMALIZE && $DOREMOVESILENCE; then
		sox $AUDIOFILE -b 16 -e signed-integer $NEWAUDIOFILENAME.wav norm vol 0.99 remix - silence 1 0.0001 0% fade 0.005 rate 44100 dither
		...
		sox $AUDIOFILE -b 16 -e signed-integer $NEWAUDIOFILENAME.wav vol 0.99 remix - silence 1 0.0001 0% fade 0.005 rate 44100 dither
	elif ! $DOAUDIONORMALIZE && ! $DOREMOVESILENCE; then
